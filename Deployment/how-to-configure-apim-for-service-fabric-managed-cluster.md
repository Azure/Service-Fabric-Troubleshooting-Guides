...
- Service Fabric managed cluster with `autoGeneratedDomainNameLabelScope` set to `ResourceGroupReuse`. This provides a static certificate configuration for the cluster server certificate (auto-rotates every 90 days without manual intervention). **MUST be currently be configured at cluster creation time** - cannot be currently be added to existing clusters. Updating existing clusters may be supported in future with [Set-AzServiceFabricManagedCluster](https://learn.microsoft.com/powershell/module/az.servicefabric/set-azservicefabricmanagedcluster)
...
> [!IMPORTANT]
> **Deployment-Time Configuration Required**: The `autoGeneratedDomainNameLabelScope` property **MUST currently be configured during initial cluster creation**.

## Validated Configuration

Testing has confirmed the following configuration successfully deploys a Service Fabric managed cluster with the new FQDN format:

**FQDN Format Generated**: `<cluster-name>.<generated-string>.<region>.sfmc.io`

**Example**: `sfmc-migration-test.bbg6daa3fqaggeap.centralus.sfmc.io`

### Template Configuration

**API Version**: `2024-09-01-preview` or later
...
> [!IMPORTANT]
> **Public CAs typically REFUSE to issue Client Authentication EKU** due to CA/Browser Forum baseline requirements. This approach realistically requires a private/enterprise CA.
...
#### Alternative: sf-managed-connect Script (Recommended for SFMC)

For Service Fabric Managed Clusters, the `sf-managed-connect` helper script provides a simpler connection method that automatically queries Azure for cluster server certificate thumbprints, eliminating the need to have the server certificate in your local store.

**Requirements**:

- PowerShell with Az module authenticated
- Client certificate with Client Authentication EKU in CurrentUser\My
- Script from [powershellscripts/servicefabric](https://github.com/jagilber/powershellscripts/tree/master/servicefabric)

**Download and Usage**:

```powershell
# Download script if not present
$scriptUrl = "https://raw.githubusercontent.com/jagilber/powershellscripts/master/servicefabric/sf-managed-connect.ps1"
$scriptPath = "$env:TEMP\sf-managed-connect.ps1"
Invoke-WebRequest -Uri $scriptUrl -OutFile $scriptPath

# Connect with named parameters (always use named parameters for reliability)
& $scriptPath `
    -clustername "sfmcapim" `
    -thumbprint "0123456789ABCDEF0123456789ABCDEF01234567" `
...
**Important**: Always use **named parameters** explicitly. Positional parameters may cause ambiguous parameter binding issues.
...
<!-- todo: update link to master branch when available -->
- Service Fabric managed cluster with DomainNameLabel configuration: [SF-Managed-Standard-SKU-1-NT-DomainNameLabel](https://github.com/jagilber/service-fabric-cluster-templates/blob/sfmcapim/SF-Managed-Standard-SKU-1-NT-DomainNameLabel/azuredeploy.json)
<!-- todo: update link to master branch when available -->
