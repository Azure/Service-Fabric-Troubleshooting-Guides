...
## Optional: Static FQDN with `domainNameLabel`

SFMC clusters default to a GUID-based FQDN. Set `autoGeneratedDomainNameLabelScope: "ResourceGroupReuse"` at cluster creation for a readable FQDN (`<cluster-name>.<hash>.<region>.sfmc.io`). Does not affect certificate validation - purely cosmetic.

See [How to Configure SFMC with Static FQDN](how-to-configure-service-fabric-managed-cluster-static-fqdn.md).
